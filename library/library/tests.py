...
class TestAuthorViewSet(TestCase):
    ...
    def test_create_guest(self):
        ...
        request = factory.post(
            "/api/authors/", {"first_name": "Михаил", "last_name": "Лермонтов", "birthday_year": 1814}, format="json"
        )
    ...
    def test_edit_admin(self):
        ...
        response = client.put(
            f"/api/authors/{author.uuid}/", {"last_name": "Грин", "first_name": "Александр", "birthday_year": 1880}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class TestToDoViewSet(APITestCase):
    ...
    def test_edit_admin(self):
        ...
        response = self.client.put(
            f"/api/todos/{todo.id}/",
            {"todo_text": "some new text", "todo_author": todoauthor.id, "project": todo.project.uuid},
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_edit_admin_mixers(self):
        ...
        response = self.client.put(
            f"/api/todos/{todo.id}/",
            {"todo_text": "some new text", "todo_author": admin.id, "project": todo.project.uuid},
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        todo = ToDo.objects.get(id=todo.id)
        self.assertEqual(todo.todo_text, "some new text")
